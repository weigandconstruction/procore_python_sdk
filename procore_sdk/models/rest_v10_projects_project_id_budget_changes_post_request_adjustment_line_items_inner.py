...
class RestV10ProjectsProjectIdBudgetChangesPostRequestAdjustmentLineItemsInner(BaseModel):
    """
    ...
    type: Optional[StrictStr] = Field(default=None, description="Used to identify type of line item. id uniquess is guaranteed per type.  NOTE: To add an Adjustment Line Item with the type budget_change for an adjustment_number, an Adjustment Line item with the type change_event must be created for the same adjustment_number.")
    ...
    __properties: ClassVar[List[str]] = ["ref", "adjustment_number", "wbs_code_id", "description", "comment", "calculation_strategy", "quantity", "type", "uom", "unit_cost", "amount", "change_event_line_item_id"]
