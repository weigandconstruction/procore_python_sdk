...
class PunchItem10(BaseModel):
    ...
    images: Optional[List[StrictStr]] = Field(default=None, description="Punch Item Images. *DEPRECATED. Please use attachments instead To upload images you must upload the entire payload as `multipart/form-data` content-type and specify each parameter as form-data together with `punch_item[images][0]` as files. `punch_item[images][0]` and `punch_item[images][1]` and so forth if you want to attach multiple images.")
