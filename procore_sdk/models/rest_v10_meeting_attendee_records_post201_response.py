...
class RestV10MeetingAttendeeRecordsPost201Response(BaseModel):
    """
    Meeting Attendee record
    """ # noqa: E501
    id: Optional[StrictInt] = Field(default=None, description="Meeting Attendee record ID")
    meeting_id: Optional[StrictInt] = Field(default=None, description="ID of the corresponding Meeting")
    status: Optional[StrictStr] = Field(default=None, description="Attendance status")
    login_information: Optional[RestV10ProjectsProjectIdWorkLogsGet200ResponseInnerCreatedBy] = None
    __properties: ClassVar[List[str]] = ["id", "meeting_id", "status", "login_information"]
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['Present', 'Absent', 'For Distribution Only', 'Conference']):
            raise ValueError("must be one of enum values ('Present', 'Absent', 'For Distribution Only', 'Conference')")
        return value
