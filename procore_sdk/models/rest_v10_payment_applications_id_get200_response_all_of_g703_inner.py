...
class RestV10PaymentApplicationsIdGet200ResponseAllOfG703Inner(BaseModel):
    """
    Payment Application (Owner Invoice) Contract Item
    """ # noqa: E501
    id: Optional[StrictInt] = Field(default=None, description="ID")
    origin_id: Optional[StrictInt] = Field(default=None, description="ID for the record this item originated from. Only present if item type is payment_application_line_item.")
    origin_type: Optional[StrictStr] = Field(default=None, description="Type of the record this item originated from. Only present if item type is payment_application_line_item.")
    added_from_source: Optional[StrictStr] = Field(default=None, description="String representing the type of object this line came from. Only present if item type is payment_application_line_item.")
    added_from_source_id: Optional[StrictInt] = Field(default=None, description="ID for the source object this line came from. Only present if item type is payment_application_line_item.")
    ...
    def origin_type_validate_enum(cls, value):
        ...
        if value not in set(['LineItem', 'ChangeOrderPackage']):
            raise ValueError("must be one of enum values ('LineItem', 'ChangeOrderPackage')")
        return value

    @field_validator('added_from_source')
    def added_from_source_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['contract', 'change_order']):
            raise ValueError("must be one of enum values ('contract', 'change_order')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "added_from_source": obj.get("added_from_source"),
            "added_from_source_id": obj.get("added_from_source_id"),
