...
class Body2(BaseModel):
    ...
    status: StrictStr = Field(description="Subcontractor SOV status. Admin users or users with granular permissions to update the contract can chan ge the status if the contract has no requisitions (sub invoices) or approved commitment change orders. Bill recipients can change the status to 'under_review' if the current status is 'draft' or 'revise_and_resubmit.'")
    __properties: ClassVar[List[str]] = ["project_id", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['draft', 'revise_and_resubmit', 'under_review', 'approved']):
            raise ValueError("must be one of enum values ('draft', 'revise_and_resubmit', 'under_review', 'approved')")
        return value
